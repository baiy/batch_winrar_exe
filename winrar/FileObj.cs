...
namespace winrar
{
    ...
    * *******************************************************************************/
    ...
    public class FileObj : IDisposable
    {
        ...
        #region 将指定文件夹下面的所有内容copy到目标文件夹下面 果目标文件夹为只读属性就会报错。
         /// <summary>
        /// 指定文件夹下面的所有内容移动到目标文件夹下面
        /// </summary>
        /// <param name="srcPath">原始路径</param>
        /// <param name="aimPath">目标文件夹</param>
        public static void MoveDir(string directorySource, string directoryTarget)
        {
            //检查是否存在目的目录
            ...
            {
                System.IO.Directory.Move(Path.Combine(directorySource, dir.Name), Path.Combine(directoryTarget, dir.Name));
            }
        }
        #endregion
